...
export default function ImagesPage() {
  ...
  const load = useCallback(async (p: number, projId: string) => {
    setLoading(true);
    try {
      const res = await mediaService.listImages(p, 20, projId);
      setItems(res.items);
      setPages(res.pages);
      setPage(res.page);
    } catch {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const img = await mediaService.createImage({ title, prompt: prompt || null, project_id: effectiveProjectId });
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImportProgress(0);
    try {
      await mediaService.importImages([...files], effectiveProjectId, setImportProgress);
      toast.success(`${files.length} image(s) imported!`);
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await mediaService.deleteImage(deleteId);
  ...
  const actionBtns = (
    <div className="d-flex align-items-center gap-2">
      <input ref={importRef} type="file" accept="image/*" multiple hidden onChange={handleImport} />
  ...
  return (
    <div className="row g-3">
      ...
      <div className="col-12 col-md-9">
        ...
        {loading ? <Loader /> : items.length === 0 ? (
          <div className="stat-card text-center py-5" style={{ color: "var(--muted)" }}>
            <MdImage size={48} className="mb-2" style={{ opacity: 0.3 }} />
            <p className="mb-0">No images found for this project.</p>
          </div>
        ) : (
          <>
            <div className="row g-3">
              {items.map((item) => (
                <div key={item.id} className="col-sm-6 col-lg-4">
                  <div className="card-custom" onClick={() => navigate(`/images/${item.id}`)}>
                    ...
                    <div className="card-custom-body">
                      <div className="card-custom-icon" style={{ background: "#10b98118", color: "#10b981" }}><MdImage size={22} /></div>
      ...
      <Modal show={showCreate} onHide={() => setShowCreate(false)} centered>
        <Modal.Header closeButton><Modal.Title>New Image</Modal.Title></Modal.Header>
      ...
      <ConfirmModal show={!!deleteId} message="Delete this image?" onConfirm={handleDelete} onCancel={() => setDeleteId(null)} />
    </div>
  );
}
